...
def SensatUrban_subset_metrics(dataset, predictions, targets, val_proportions=None):
    # Confusions for subparts of validation set
    if not val_proportions:
        val_proportions = dataset.proportions
    # Confs: (B, C, C)
...
def SensatUrban_voting_metrics(dataset, validation_probs, val_proportions=None):
    if not val_proportions:
        val_proportions = dataset.proportions
    Confs = []
